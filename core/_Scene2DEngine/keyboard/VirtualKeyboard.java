...
public class VirtualKeyboard implements OnscreenKeyboard, InputProcessor {
	...
	public VirtualKeyboard(SpriteBatch batch) {
		...
		_width = _screenWidth;
		_height = (_screenWidth / _screenHeight > 1.0f ? (_screenHeight * ((_size == Size.Large) ? .5f
				: .6f))
				: (_screenHeight * ((_size == Size.Large) ? .4f : .45f)));
		_duration = 0.1f;
		_telex = new TelexIM();
	...
	public void setText(String content) {
		...
		for (int i = 0; i < content.length(); i++) {
			...
			if (_language == 1 && isAccent(character)
					&& _textBuffer.getCurrentChar() != 32) {
				String[] temp = _textBuffer.toString().split("\n");
				temp = temp[temp.length - 1].split(" ");
				String currentWord = temp[temp.length - 1];
				if (currentWord.length() > 0) {
					char accent = _telex.getAccentMark(character, '\0',
							currentWord);
					if (accent >= '0' && accent <= '9') {
						String vietWord = VietKeyInput.toVietWord(currentWord,
								accent);
	...
	private void _initDefaultQwerty() {
		...
		for (int i = 0; i < DEFAULT_QWERTY[0].length; i++) {
			_defaultQwerty[i] = new KeyButton(DEFAULT_QWERTY[0][i], paddingX
					+ i * (w + px), _pos.y + _height - paddingY - h, w, h, this);
		...
		for (int i = 0; i < DEFAULT_QWERTY[1].length; i++) {
			_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[1][i], px1
					+ i * (w + px), _pos.y + _height - paddingY - h - (h + py),
					w, h, this);
			_defaultQwerty[i + count].setBackground(_normalButton);
			_defaultQwerty[i + count]
					.setPressedBackground(_pressedNormalButton);
		...
		for (int i = 0; i < DEFAULT_QWERTY[2].length; i++) {
			if (i == 0) {
				// shift button
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[2][i],
						paddingX, _pos.y + _height - paddingY - h - 2
								* (h + py), sw, sh, this);
				_defaultQwerty[i + count].setBackground(_specialButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedSpecialButton);
				_defaultQwerty[i + count].setFont(_font);
				...
				// delete button
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[2][i],
						this._width - sw - paddingX, _pos.y + _height
								- paddingY - h - 2 * (h + py), sw, sh, this);
				_defaultQwerty[i + count].setBackground(_specialButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedSpecialButton);
				_defaultQwerty[i + count].setFont(_font);
				...
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[2][i],
						px2 + (i - 1) * (w + px), _pos.y + _height - paddingY
								- h - 2 * (h + py), w, h, this);
				_defaultQwerty[i + count].setBackground(_normalButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedNormalButton);
		...
		for (int i = 0; i < DEFAULT_QWERTY[3].length; i++) {
			if (i == 0) {
				// close button
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[3][i],
						paddingX, _pos.y + _height - paddingY - h - 3
								* (h + py), Math.min(sw / 2 + px2 / 2,
								1.2f * sw), sh, this);
				_defaultQwerty[i + count].setBackground(_specialButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedSpecialButton);
				...
				// mode button
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[3][i],
						tempX, _pos.y + _height - paddingY - h - 3 * (h + py),
						w, h, this);
				_defaultQwerty[i + count].setBackground(_specialButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedSpecialButton);
				...
				// space button
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[3][i],
						px2 + 2 * (w + px), _pos.y + _height - paddingY - h - 3
								* (h + py), 3 * w + 2 * px, h, this);
				_defaultQwerty[i + count].setBackground(_normalButton);
				_defaultQwerty[i + count]
						.setPressedBackground(_pressedSpecialButton);
				_defaultQwerty[i + count].setFont(_font);
				...
				_defaultQwerty[i + count] = new KeyButton(DEFAULT_QWERTY[3][i],
						this._width - Math.min(sw / 2 + px2 / 2, 1.2f * sw)
								- paddingX, _pos.y + _height - paddingY - h - 3
								* (h + py), Math.min(sw / 2 + px2 / 2,
								1.2f * sw), sh, this);
				_defaultQwerty[i + count].setBackground(_specialButton);
	...
	private void _initDefaultNumbersSymbols(KeyButton[] keys, String[][] pattern) {
		...
		for (int i = 0; i < pattern[0].length; i++) {
			keys[i] = new KeyButton(pattern[0][i], paddingX + i * (w + px),
					_pos.y + _height - paddingY - h, w, h, this);
		...
		for (int i = 0; i < pattern[1].length; i++) {
			keys[i + count] = new KeyButton(pattern[1][i], px1 + i * (w + px),
					_pos.y + _height - paddingY - h - (h + py), w, h, this);
		...
		for (int i = 0; i < pattern[2].length; i++) {
			if (i == pattern[2].length - 1) {
				// delete button
				keys[i + count] = new KeyButton(pattern[2][i], this._width
						- 1.5f * w - px2, _pos.y + _height - paddingY - h - 2
						* (h + py), 1.5f * w, sh, this);
				...
				keys[i + count] = new KeyButton(pattern[2][i], px2 + i
						* (w + px), _pos.y + _height - paddingY - h - 2
						* (h + py), w, h, this);
				keys[i + count].setBackground(_normalButton);
				keys[i + count].setPressedBackground(_pressedNormalButton);
				keys[i + count].setPopupBackground(_popupButton);
		...
		for (int i = 0; i < pattern[3].length; i++) {
			if (i == 0) {
				// close button
				keys[i + count] = new KeyButton(pattern[3][i], paddingX, _pos.y
						+ _height - paddingY - h - 3 * (h + py), Math.min(sw
						/ 2 + px3 / 2, 1.2f * sw), sh, this);
				keys[i + count].setBackground(_specialButton);
				keys[i + count].setPressedBackground(_pressedSpecialButton);
				keys[i + count].setFont(_font);
				...
				// mode button
				keys[i + count] = new KeyButton(pattern[3][i], tempX, _pos.y
						+ _height - paddingY - h - 3 * (h + py), w, h, this);
				keys[i + count].setBackground(_specialButton);
				keys[i + count].setPressedBackground(_pressedSpecialButton);
				...
				// space button
				keys[i + count] = new KeyButton(pattern[3][i], px3 + 2
						* (w + px), _pos.y + _height - paddingY - h - 3
						* (h + py), 3 * w + 2 * px, h, this);
				keys[i + count].setBackground(_normalButton);
				keys[i + count].setPressedBackground(_pressedSpecialButton);
				keys[i + count].setFont(_font);
				...
				keys[i + count] = new KeyButton(pattern[3][i], this._width
						- Math.min(sw / 2 + px3 / 2, 1.2f * sw) - paddingX,
						_pos.y + _height - paddingY - h - 3 * (h + py),
						Math.min(sw / 2 + px3 / 2, 1.2f * sw), sh, this);
				keys[i + count].setBackground(_specialButton);
				keys[i + count].setPressedBackground(_pressedSpecialButton);
				keys[i + count].setFont(_font);
	...
	private void _initDefaultNumbersSymbols1() {
		...
		_initDefaultNumbersSymbols(_defaultNumbersSymbols1,
				DEFAULT_NUMBERS_SYMBOLS_1);
	}

	private void _initDefaultNumbersSymbols2() {
		_defaultNumbersSymbols2 = new KeyButton[35];
		_initDefaultNumbersSymbols(_defaultNumbersSymbols2,
				DEFAULT_NUMBERS_SYMBOLS_2);
	}
	...
	private void _load() {
		...
		if (_size == Size.Small) {
			_font = new BitmapFont(Gdx.files.internal("keyboard/font-20.fnt"),
					false);
			_font.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_font.setColor(Color.BLACK);
		} else if (_size == Size.Medium) {
			_font = new BitmapFont(Gdx.files.internal("keyboard/font-30.fnt"),
					false);
			_font.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_font.setColor(Color.BLACK);
		} else {
			_font = new BitmapFont(Gdx.files.internal("keyboard/font-45.fnt"),
					false);
			_font.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_font.setColor(Color.BLACK);
		...
		if (_size == Size.Small) {
			_popupFont = new BitmapFont(
					Gdx.files.internal("keyboard/popup-font-30.fnt"), false);
			_popupFont.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_popupFont.setColor(Color.WHITE);
		} else if (_size == Size.Medium) {
			_popupFont = new BitmapFont(
					Gdx.files.internal("keyboard/popup-font-50.fnt"), false);
			_popupFont.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_popupFont.setColor(Color.WHITE);
		} else {
			_popupFont = new BitmapFont(
					Gdx.files.internal("keyboard/popup-font-60.fnt"), false);
			_popupFont.getRegion().getTexture()
					.setFilter(TextureFilter.Linear, TextureFilter.Linear);
			_popupFont.setColor(Color.WHITE);
		}
		...
		if ((_screenWidth / _screenHeight) > 1) {
			if (_size == Size.Small) {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-15.fnt"), false);
			} else if (_size == Size.Medium) {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-26.fnt"), false);
			} else {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-40.fnt"), false);
			}
		} else {
			if (_size == Size.Small) {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-10.fnt"), false);
			} else if (_size == Size.Medium) {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-18.fnt"), false);
			} else {
				_modeFont = new BitmapFont(
						Gdx.files.internal("keyboard/mode-font-35.fnt"), false);
			}
		}
		_modeFont.getRegion().getTexture()
				.setFilter(TextureFilter.Linear, TextureFilter.Linear);
	...
	private void _init() {
		...
		if (_size == Size.Large) {
			_largeAtlas = _assetManager.get("keyboard/image_large.pack",
					TextureAtlas.class);
		}
		...
		if (_size == Size.Large)
			_inShiftButton = _largeAtlas
					.findRegion("default_icon_shift_inactive");
		else
			_inShiftButton = _atlas.findRegion("default_icon_shift_inactive");
		if (_size == Size.Large)
			_acShiftButton = _largeAtlas
					.findRegion("default_icon_shift_active");
		else
		...
		_background.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_closeButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_deleteButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_enterButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_inShiftButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_acShiftButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		_okButton.getTexture().setFilter(TextureFilter.Linear,
				TextureFilter.Linear);
		...
		_standardFX = _assetManager
				.get("keyboard/fx_standard.wav", Sound.class);

		_normalButton = _atlas.createPatch("default_btn_keyboard_key_normal");
		_pressedNormalButton = _atlas
				.createPatch("default_btn_special_key_normal");
		_specialButton = _atlas.createPatch("default_btn_keyboard_key_pressed");
		_pressedSpecialButton = _atlas
				.createPatch("default_btn_special_key_pressed");
		_popupButton = _atlas.createPatch("default_btn_popup_key");

		_editText = new EditText(0, 0, "", _font, _camera, _viewport);
		_editText.setBackground(_atlas.createPatch("edit_text"), _width - 4,
				_font.getCapHeight() * 2);
		...
		_iconScale = _font.getCapHeight()
				/ Math.abs(_deleteButton.getRegionHeight());
	...
	public void setMode(int mode) {
		...
		if (_mode == KeyboardConfig.MULTI_LINE) {
			...
			_defaultNumbersSymbols1[_enterButtonNumberIndex]
					.setIcon(_enterButton);
			_defaultNumbersSymbols2[_enterButtonNumberIndex]
					.setIcon(_enterButton);
	...
	public void addKeyCode(char character) {
		...
		if (_language == 1 && isAccent(character)
			...
			if (currentWord.length() > 0) {
				char accent = _telex
						.getAccentMark(character, '\0', currentWord);
				if (accent >= '0' && accent <= '9') {
					String vietWord = VietKeyInput.toVietWord(currentWord,
							accent);
					if (!vietWord.equals(currentWord)) {
	...
	public boolean isAccent(char character) {
		if (character == 'S' || character == 's' || character == 'F'
				|| character == 'f' || character == 'R' || character == 'r'
				|| character == 'X' || character == 'x' || character == 'J'
				|| character == 'j' || character == 'A' || character == 'a'
				|| character == 'E' || character == 'e' || character == 'O'
				|| character == 'o' || character == 'W' || character == 'w'
				|| character == 'D' || character == 'd' || character == 'Z'
				|| character == 'z')
			return true;
		else
			return false;
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		_camera.unproject(pos, _viewport.x, _viewport.y, _viewport.width,
				_viewport.height);
		if (isShowing()) {
			if (pos.x >= _pos.x && pos.x <= _pos.x + _width && pos.y >= _pos.y
					&& pos.y <= _pos.y + _height) {
				if (button == 0) {
					for (KeyButton keyButton : _currentKeyBoard) {
						if (keyButton != null)
							keyButton.isPressed(pos.x, pos.y, _viewport.x,
									_viewport.y, _viewport.width,
									_viewport.height);
					}
					if (_type != KeyboardConfig.PASSWORD
							&& _editText.contains(pos.x, pos.y, _viewport.x,
									_viewport.y, _viewport.width,
									_viewport.height)) {
						Vector2 anchor = _editText.getTextPosition();
						float x = anchor.x + _viewport.x;
						float y = anchor.y + _viewport.y;
						_textBuffer.catchCurrsor(pos.x - x, y - pos.y, 1, 1,
								_font);
					}
				}
				return true;
	...
	private class KeyButton {
		...
		public boolean isPressed(float x, float y, float viewportX,
			...
			if (bound.contains(x, y)) {
				...
				if (_textBuffer.length() >= _maxLength
						&& !name.equals("$Delete$"))
					return true;
		...
		public void render(SpriteBatch batch) {
			if (isPressed) {
				...
				if (popupBack != null) {
					popupBack.draw(batch,
							((pos.x - 1.5f * dimension.x / 3 < 0) ? 0
									: (pos.x - 1.5f * dimension.x / 3)), pos.y
									+ dimension.y, 1.5f * dimension.x,
							1.5f * dimension.y);
				}
			} else {
			...
			if (!(name.length() > 1 && name.startsWith("$") && name
					.endsWith("$"))) {
				...
				font.draw(batch, name, pos.x + (dimension.x - bound.width) / 2,
						pos.y + (dimension.y + bound.height) / 2);
				if (isPressed && popupBack != null) {
					bound = popupFont.getBounds(name);
					popupFont.draw(batch, name, ((pos.x - 1.5f * dimension.x
							/ 3 < 0) ? 0 : (pos.x - 1.5f * dimension.x / 3))
							+ (1.5f * dimension.x - bound.width) / 2, pos.y
							+ dimension.y + (1.5f * dimension.y + bound.height)
							/ 2);
				}
			} else {
				if (icon != null) {
					batch.draw(
							icon,
							pos.x
									+ (dimension.x - Math.abs(icon
											.getRegionWidth() * scale)) / 2,
							pos.y
									+ (dimension.y - Math.abs(icon
											.getRegionHeight() * scale)) / 2,
							Math.abs(icon.getRegionWidth()) * scale,
							Math.abs(icon.getRegionHeight()) * scale);
					if (name.equals("$Shift$") && parent.getShift() == 2) {
						batch.draw(
								subIcon,
								pos.x
										+ (dimension.x - Math.abs(subIcon
												.getRegionWidth() * scale)) / 2,
								pos.y
										+ (dimension.y - Math.abs(subIcon
												.getRegionHeight() * scale))
										/ 2, Math.abs(subIcon.getRegionWidth())
										* scale,
				...
				if (name.equals("$Language$")) {
					TextBounds bound = font
							.getBounds(parent._getLanguageMode());
					font.draw(batch, parent._getLanguageMode(), pos.x
							+ (dimension.x - bound.width) / 2, pos.y
							+ (dimension.y + bound.height) / 2);
					return;
				}

				if (name.equals("$Mode$") || name.equals("$Mode1$")) {
					TextBounds bound = font.getBounds(parent._getModeLabel());
					font.draw(batch, parent._getModeLabel(), pos.x
							+ (dimension.x - bound.width) / 2, pos.y
							+ (dimension.y + bound.height) / 2);
					return;
				}
				...
				if (name.equals("$Mode2$")) {
					...
					font.draw(batch, "ABC", pos.x + (dimension.x - bound.width)
							/ 2, pos.y + (dimension.y + bound.height) / 2);
					return;
				}
			}
		}
	...
	protected class SimpleCharSequence implements CharSequence {
		...
		public Vector3 getCursorInfo(float scaleX, float scaleY, BitmapFont font) {
			...
			return new Vector3(bound.width * scaleX, font.getCapHeight()
					* scaleY, count);
		}

		public void catchCurrsor(float deltaX, float deltaY, float scaleX,
				float scaleY, BitmapFont font) {
			if (deltaY < 0)
				return;
			cursor = -1;
